...
$(".expandIcon").on("click", function(){ 
  var $sibling = $(this).siblings("p, h5");
  $sibling.toggle();
  if($(this).text() == "+"){
    $(this).text("-");
  }else{
...
for(let i = 0; i < 9; i++){
   $.getJSON('https://wind-bow.gomix.me/twitch-api/streams/' + $('h4:eq(' + i + ')').text() + '?callback=?', function(data){
      if(data.stream == null){
        $('h4:eq(' + i + ')').parent().removeClass("online").addClass("offline");
        $('h4:eq(' + i + ')').siblings(".streamStatus").text("Offline");
      }else{
        $('h4:eq(' + i + ')').parent().removeClass("offline").addClass("online");
        $('h4:eq(' + i + ')').siblings(".streamStatus").text("Streaming: " + data.stream.game);
      }
   });
 }
